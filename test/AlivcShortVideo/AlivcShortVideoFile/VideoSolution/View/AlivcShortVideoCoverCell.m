...
- (void)setModel:(AlivcShortVideoBasicVideoModel *)model {
   ...
   // _model = (AlivcQuVideoModel *) model;
    ...
    if (model) {
        ...
        if (model.coverUrl && model.coverUrl.length >0) {
            [self.imageView sd_setImageWithURL:[NSURL URLWithString:model.coverUrl] placeholderImage:[UIImage imageNamed:@""] completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
        ...
        }else if (model.firstFrameUrl && model.firstFrameUrl.length >0){
