...
- (void)loadSDKInfo{
    ...
    versionLabel.text = [NSString stringWithFormat:@"趣视频版本号【V%@】",versionString];
    ...
    NSString *allVersionString = [NSString stringWithFormat:@"包含短视频SDK 版本号【V%@】/ 播放器SDK 版本号【V%@】",shortVideoSDKVersionString,playSDKVersionString];
...
- (IBAction)changeUser:(id)sender {
    ...
    if (self.type == AlivcUserVCTypeQuVideo) {
        if (_isPublishing) {
            [MBProgressHUD showMessage:@"当前有视频正在发布中,无法更换用户" inView:self.view];
        }else{
            UIAlertView *alertView = [[UIAlertView alloc]initWithTitle:@"警告⚠️" message:@"更换用户后你所有的个人视频与信息将丢失,请谨慎操作!" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"更换", nil];
            [alertView show];
        }
    }else{
        [self changeUser];
